...
class NotesAccordion extends Component {
  ...
  renderHeaderButtons() {
    const { hideAssignButton, hideNewButton, stripes } = this.props;

    return (
      <>
        {!hideAssignButton
        && (stripes.hasPerm('ui-notes.item.assign-unassign')
          || stripes.hasPerm('ui-notes.item.assign-unassign.execute'))(
            <Button
...
export default withStripes(NotesAccordion);
